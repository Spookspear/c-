...
namespace ToolbarOfFunctions_CommonClasses
{
	public static class CommonExcelClasses
	{
		...
		public static bool isEmptyCell(Excel.Range xlCell, bool boolZeroMeansEmpty)
		{
			bool boolRetVal = false;
			if (!boolZeroMeansEmpty) {
				if (xlCell == null || xlCell.Value2 == null || xlCell.Value2.ToString() == "")
					boolRetVal = true;

			} else {
				if (xlCell == null || xlCell.Value2 == null || xlCell.Value2.ToString() == "" || xlCell.Value2 == 0)
					boolRetVal = true;
			}
		...
		// extensability - will it work? YES: : 04-02-2019

		// this will return the col number for a range - 1gvb1
		public static int getColNum( this string strRange )
		{
			if (strRange.IndexOf(":") < 0) { strRange = strRange + "1"; }
		...
		public static int getRowNum( this string strRange )
		{
			var match = Regex.Match(strRange, @"(?<col>[A-Z]+)(?<row>\d+)");
			string colStr = match.Groups["col"].ToString();
		...
		public static string getColLtr( this int intColumnNumber )
		{

			int intDiv = intColumnNumber;
		...
		public static void formatCells( Excel.Worksheet Wks, decimal decRow, decimal decCol, string strDoWhat )
		{

			// need to loop for cols - 1gvb2

			Excel.Range xlCell;
			xlCell = Wks.Cells[decRow, decCol];

			if (strDoWhat == "BlackAndWhite")
			{
			...
			if (strDoWhat == "Center" || strDoWhat == "Centre")
			{

				xlCell.Cells.HorizontalAlignment = Excel.XlHAlign.xlHAlignCenter;
				xlCell.Cells.VerticalAlignment = Excel.XlHAlign.xlHAlignCenter;
			}
		...
		public static void formatCellsOld(Excel.Worksheet Wks, decimal intStartRow, decimal intLastRow, decimal intStartColum, decimal intNumColums, string strDoWhat)
		{
		...
		public static void clearFormattingRange(Excel.Worksheet Wks)
		{
			...
			// string strRange = "A1:" + getExcelColumnLetter(getLastCol(Wks)) + getLastRow(Wks);		// 1gvb3
			string strRange = "A1:" + getLastCol(Wks).getColLtr() + getLastRow(Wks);
		...
		{
			// this will sort the sheet on the 1st column getColLtr
			#region [Define range dynamically]
			// string strRange = "A2:" + getExcelColumnLetter(getLastCol(Wks)) + getLastRow(Wks);
			string strRange = "A2:" + getLastCol(Wks).getColLtr() + getLastRow(Wks);			// 1gvb3
			#endregion
		...
		public static void sortSheetWorking(Excel.Worksheet Wks)
		{
			...
			#region [Define range dynamically]
			// string strRange = "A2:" + getExcelColumnLetter(getLastCol(Wks)) + getLastRow(Wks);	// 1gvb3
			string strRange = "A2:" + getLastCol(Wks).getColLtr() + getLastRow(Wks);
			#endregion
		...
		public static string createFormula(string strDeltaCol, int intRowStart, int intRowEnd )
		{
			...
			if (strValue.Length > 0)
			{
				if (strValue.Length == 19)
				{
					...
					if (CommonExcelClasses.IsDate(strValue))
					{
						...
						boolRetVal = (strDayOfWeek == "Monday" || strDayOfWeek == "Tuesday" || strDayOfWeek == "Wednesday" || strDayOfWeek == "Thursday" || strDayOfWeek == "Friday" || strDayOfWeek == "Saturday" || strDayOfWeek == "Sunday");
					}
				}
			}
		...
		public static void colourCells(Excel.Worksheet Wks, decimal decSourceRow, string strDoWhat, decimal decNoCheckCols, Color clrWhichColourFore, Color clrWhichColourBack, bool boolTestCode)
		{
			...
			*/
			...
			for (int intColCount = intStartColumToCheck; intColCount <= decNoCheckCols; intColCount++)
			{
				xlCell = Wks.Cells[decSourceRow, intColCount];
		...
		public static bool checkEmptyRange(Worksheet Wks, string strRange)
		{
			...
			for (int x = intArrCoords[_ROW1]; x <= intArrCoords[_ROW2]; x++)
			{
				...
				{
					if (!CommonExcelClasses.isEmptyCell(Wks.Cells[x, y],false))
					{
						intNotEmpty++;
						break;
					}
